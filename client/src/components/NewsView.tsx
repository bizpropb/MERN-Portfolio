...
const NewsView: React.FC = () => {
  ...
  const fetchArticleBySlug = async (articleSlug: string) => {
    try {
      const response = await fetch(`http://localhost:5000/api/news/${articleSlug}`, {
  ...
  const fetchLatestArticle = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/news/latest?limit=1`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await response.json();
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <article className="bg-white dark:bg-gray-800 rounded-lg shadow-md px-8 py-4 mb-12">
          <div className="prose prose-md dark:prose-invert max-w-none 
                          ...
                          prose-h1:text-2xl prose-h1:font-bold prose-h1:mb-5 prose-h1:mt-1
      ...
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-6 right-6 bg-purple-600 hover:bg-purple-700 text-white p-3 rounded-full shadow-lg transition-all duration-300 hover:scale-110 z-50"
